...
-(void) prepareFrames{
    ...
    for (int i = 1; i <=1; ++i) {
        [standingAnimationFrames addObject: [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:[NSString stringWithFormat:@"kingar_stand_0%d.png",i]]];
    }
...
-(id) summonWithParameters: (int)x y: (int)y state: (State) state facing: (Facing) facing {
    if (( self = [super init])) {
        ...
        if (self.state == WALKING) {
            ...
            self.character = [CCSprite spriteWithSpriteFrameName:@"kingar_stand_01.png"];
            [self runStandAction];
        }
