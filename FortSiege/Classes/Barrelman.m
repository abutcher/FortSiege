...
-(void) prepareFrames{
    ...
    for (int i = 1; i <= 6; ++i) {
        [walkingAnimationFrames addObject: [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName: [NSString stringWithFormat:@"barrelman_walk_0%d.png", i]]];
    }
    ...
    for (int i = 1; i<=3; ++i) {
        [attackingAnimationFrames addObject: [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:[NSString stringWithFormat:@"barrelman_attack_0%d.png",i]]];
    }
...
-(id) init {
    ...
    if ( ( self = [super init] )) {
        ...
        self.character = [CCSprite spriteWithSpriteFrameName:@"barrelman_walk_01.png"];
        [self runWalkAction];
